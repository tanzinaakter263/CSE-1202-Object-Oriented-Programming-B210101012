...
void temp :: createAccount(){
	...
	file<<accNo<<"\t"<<name<<"\t"<<Fname<<"\t"<<nid<<"\t"<<P_no<<"\t"<<email<<"\t"<<amount<<endl;
	file.close();
}
...
void temp :: depositAmount(){
	...
	file>>accNo>>name>>Fname>>nid>>P_no>>email>>amount;

	while(!file.eof()) {
		if(accNo==search){
			cout<<"\ncurrent amount :: "<<amount;
			amount+=newAmount; // Fixed typo: changed amount+=amount to amount+=newAmount
			cout<<"\nupdated amount :: "<<amount<<endl;
			file1<<accNo<<"\t"<<name<<"\t"<<Fname<<"\t"<<nid<<"\t"<<P_no<<"\t"<<email<<"\t"<<amount<<endl;
		}else{
			file1<<accNo<<"\t"<<name<<"\t"<<Fname<<"\t"<<nid<<"\t"<<P_no<<"\t"<<email<<"\t"<<amount<<endl;
		}
		file>>accNo>>name>>Fname>>nid>>P_no>>email>>amount;
